 ...
 *
 *              外部晶振为25M的时候, 推荐值: plln = 104, pllm = 5, pllp = 1, pllq = 2.
 *              得到:Fvco = 25 * (104 / 5) = 520Mhz
 *                   Fsys = pll1_p_ck = 520 / 1 = 520Mhz
 *                   Fq   = pll1_q_ck = 520 / 2 = 260Mhz
 *
 *              H723默认需要配置的频率如下:
 *              CPU频率(rcc_c_ck) = sys_d1cpre_ck = 520Mhz
 *              rcc_aclk = rcc_hclk3 = 260Mhz
 *              AHB1/2/3/4(rcc_hclk1/2/3/4) = 260Mhz
 *              APB1/2/3/4(rcc_pclk1/2/3/4) = 130Mhz
...
uint8_t sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    __HAL_PWR_VOLTAGESCALING_CONFIG(PWR_REGULATOR_VOLTAGE_SCALE0);  /* VOS[1:0] = 0, 电压调节选择Scale0 */
    ...
    /*
     *  选择PLL的输出作为系统时钟
     *  配置RCC_CLOCKTYPE_SYSCLK系统时钟,520M
     *  配置RCC_CLOCKTYPE_HCLK 时钟,260Mhz,对应AHB1，AHB2，AHB3和AHB4总线
     *  配置RCC_CLOCKTYPE_PCLK1时钟,130Mhz,对应APB1总线
     *  配置RCC_CLOCKTYPE_PCLK2时钟,130Mhz,对应APB2总线
     *  配置RCC_CLOCKTYPE_D1PCLK1时钟,130Mhz,对应APB3总线
     *  配置RCC_CLOCKTYPE_D3PCLK1时钟,130Mhz,对应APB4总线
     */
    ...
    ret = HAL_RCC_ClockConfig(&rcc_clk_init_handle, FLASH_LATENCY_3);
