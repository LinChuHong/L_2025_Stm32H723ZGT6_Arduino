...
void sdram_init(void)
{
     ...
     */
    HAL_SDRAM_ProgramRefreshRate(&g_sdram_handle, 839);                       /* 设置刷新频率计数器 */
}
...
void HAL_SDRAM_MspInit(SDRAM_HandleTypeDef *hsdram)
{
    ...
    HAL_GPIO_Init(GPIOA, &gpio_init_struct);     /* 初始化PA7 */

    gpio_init_struct.Pin = GPIO_PIN_2 | GPIO_PIN_3;
    HAL_GPIO_Init(GPIOC, &gpio_init_struct);     /* 初始化PC2,3 */
    ...
    gpio_init_struct.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 | 
                           GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15;              
    HAL_GPIO_Init(GPIOE, &gpio_init_struct);     /* 初始化PE0,1,7,8,9,10,11,12,13,14,15 */
    
    gpio_init_struct.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_11 | 
                           GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15;              
...
/**
 * @brief       从SDRAM指定地址(writeaddr + BANK5_SDRAM_ADDR)开始,连续写入n个字节
 * @param       pbuf      : 字节指针
...
/**
 * @brief       从SDRAM指定地址(readaddr + BANK5_SDRAM_ADDR)开始,连续读取n个字节.
 * @param       pbuf     : 字节指针
