...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd)
{
    ...
    if (hpcd->Instance == USB1_OTG_HS)
    {
        ...
        __HAL_RCC_USB1_OTG_HS_CLK_ENABLE();                  /* 使能USB1 OTG时钟 */
        ...
        HAL_NVIC_SetPriority(OTG_HS_IRQn, 1, 0);             /* 中断优先级设置为抢占优先级1,子优先级0 */
        HAL_NVIC_EnableIRQ(OTG_HS_IRQn);                     /* 使能OTG HS中断 */
    }
}
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
    ...
    {
        /* 设置LL驱动相关参数 */
        g_hpcd.Instance = USB1_OTG_HS;                /* 使用USB1 OTG HS */
