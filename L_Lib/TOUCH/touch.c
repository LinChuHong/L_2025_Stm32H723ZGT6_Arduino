...
/* TP_SAVE_ADDR_BASE定义触摸屏校准参数保存在SPI FLASH里面的位置(起始地址)
 * 占用空间 : 13字节.
 */
#define TP_SAVE_ADDR_BASE   (16 * 1024 * 1024 - 1024)

/**
 * @brief       保存校准参数
 * @note        参数保存在SPI FLASH芯片里面(W25Q128),起始地址为TP_SAVE_ADDR_BASE.
 *              占用大小为13字节
 * @param       无
 * @retval      无
 */
void tp_save_adjust_data(void)
{
     ...
     */
    norflash_write((uint8_t *)p, TP_SAVE_ADDR_BASE, 12);   /* 保存12个字节数据(xfac,yfac,xc,yc) */
    norflash_write(&temp, TP_SAVE_ADDR_BASE + 12, 1);      /* 保存校准值 */
}

/**
 * @brief       获取保存在SPI FLASH里面的校准值
 * @param       无
 * @retval      0，获取失败，要重新校准
 *              1，成功获取数据
 */
uint8_t tp_get_adjust_data(void)
{
     ...
     */
    norflash_read(p, TP_SAVE_ADDR_BASE, 12);           /* 读取12字节数据 */
    norflash_read(&temp, TP_SAVE_ADDR_BASE + 12, 1);   /* 读取校准状态标记 */
...
uint8_t tp_init(void)
{
    ...
    if (lcddev.id == 0x7796 || lcddev.id == 0X5310 || lcddev.id == 0X5510 || lcddev.id == 0X7789 || lcddev.id == 0X9806)  /* MCU屏或SPI接口屏，有电阻触摸屏和电容触摸屏 */
    {
        if (gt9xxx_init() == 0)             /* 初始化GT系列触摸屏成功,即当前LCD屏为电容触摸屏 */
        {
    ...
    if (lcddev.id == 0X4342 || lcddev.id == 0X4384 || lcddev.id == 0X5571 || lcddev.id == 0X8081 || lcddev.id == 0X1018)  /* gtxx系列电容触摸屏 */
    {
        gt9xxx_init();
        tp_dev.scan = gt9xxx_scan;          /* 扫描函数指向gt9xxx触摸屏扫描 */
        tp_dev.touchtype |= 0X80;           /* 电容屏 */
    ...
    else if (lcddev.id == 0X7084 || lcddev.id == 0X7016)   /* ftxx系列电容触摸屏 */
    {
